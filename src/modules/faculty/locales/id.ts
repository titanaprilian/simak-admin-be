export const id = {
  listSuccess: "Fakultas berhasil diambil",
  getSuccess: "Fakultas berhasil diambil",
  createSuccess: "Fakultas berhasil dibuat",
  updateSuccess: "Fakultas berhasil diperbarui",
  deleteSuccess: "Fakultas berhasil dihapus",
  optionsSuccess: "Opsi fakultas berhasil diambil",
  notFound: "Fakultas tidak ditemukan",
  forbiddenRole:
    "Peran {{roleName}} tidak dapat melakukan tindakan ini pada fakultas",
};
